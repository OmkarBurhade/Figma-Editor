...
export function initDrag(canvas) {
    ...
    if(!target.classList.contains('element')) return; // chek kiya element select he ki nahi 
    ...
    document.addEventListener('mousemove', (e)=>{ 
        ...
        let newY = e.clientY - canvasRect.top - offsetY
         ...
         newX = Math.max(0, Math.min(newX, canvas.clientWidth - currentEl.offsetWidth));
         newY = Math.max(0, Math.min(newY, canvas.clientHeight - currentEl.offsetHeight));
